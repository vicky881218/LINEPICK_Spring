...
//找這一筆orderlist的orderItemId拿去當orderItem的fk
public OrderList findThisOrderList(String pay_type,String pay_status,String orderlist_status,int orderlist_payment,String buyer_id) {
  return this.jdbcTemplate.queryForObject( "select orderlist_id,pay_type,pay_status,orderlist_status,orderlist_payment,buyer_id from order_item where pay_type=?,pay_status=?,orderlist_status=?,orderlist_payment=?,buyer_id=?", 
   new OrderListMapper(),pay_type,pay_status,orderlist_status,orderlist_payment,buyer_id);
}
 ...
 public int insert(OrderList orderList) throws SQLException{
  return jdbcTemplate.update(
    "insert into orderlist (ordelist_id,pay_type, pay_status,orderlist_status,orderlist_payment,buyer_id) values(?,?,?,?,?,?)",
    orderList.getOrderListId(),orderList.getPayType(),orderList.getPayStatus(),orderList.getOrderListStatus(),orderList.getOrderListPayment(),orderList.getBuyerId());
 }
}
