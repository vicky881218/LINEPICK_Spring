...
public class ReplyDAODB implements ReplyDAO {
  ...
  public Reply findOne(int seller_id) {
    ...
    Reply areply = jdbcTemplate.queryForObject("select * from reply group by seller_id = ?", new ReplyMapper(),
        seller_id);
  ...
  public Reply findOneSeller(int reply_id) {
    ...
    Reply areply = jdbcTemplate.queryForObject("select * from reply where reply_id = ?", new ReplyMapper(), reply_id);
    return areply;
  }

  public List<Reply> findAll() {
    return this.jdbcTemplate.query("select * from reply", new ReplyMapper());
  }

  public List<Reply> findAllQuestion(int reply_id) {
    return this.jdbcTemplate.query("select * from reply where reply_id=?", new ReplyMapper(), reply_id);
  }
  ...
  public int insert(Reply Reply) throws SQLException {
    return jdbcTemplate.update("insert into Reply (reply_id, reply_question, reply_answer,seller_id) values(?,?,?,?)",
        Reply.getReplyId(), Reply.getReplyQuestion(), Reply.getReplyAnswer(), Reply.getSellerId());
  }

  public int update(Reply Reply) {
    return jdbcTemplate.update("update Reply set reply_question=?, reply_answer=?,seller_id=? where reply_id =?",
